...
@Component
@Slf4j
public class SimProcessor implements ItemProcessor<User, User> {
    ...
    public User process(User user) throws InterruptedException {
        ...
        log.info("*** DONE SIM5Scaled AGGR ("+NUMAGGR+" seconds) "+(MAXTIME/86400)+" days ***");

        outFile = dir+"simResultsTest6Scaledt"+MAXTIME+"a"+NUMAGGR+".tsv";
        simToRun = new SimParams6Scaled();
        runSimSpaceOptimisedAggregated(simToRun, NUMRUNS, MAXTIME, outFile, NUMAGGR);
        log.info("*** DONE SIM6Scaled AGGR ("+NUMAGGR+" seconds) "+(MAXTIME/86400)+" days ***");
        Thread.sleep(30000);
        return user;
    }
